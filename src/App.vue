        ...
        <div v-if="view !== 'report'" class="table-wrap" :aria-busy="loading">
          <table v-if="view === 'jobs'">
            ...
            <tbody><tr v-for="job in jobs" :key="job.id">
              ...
              <td class="actions">
                <div class="row-actions"><button class="row-action" :disabled="!canWrite" @click="execute(job)"><Play :size="14" />{{ t('执行一次') }}</button>
                <button class="row-action" :disabled="loading || working || authExpired" @click="showJobLogs(job)"><FileText :size="14" />{{ t('查看任务日志') }}</button>
                <button class="row-action" :disabled="!canWrite" @click="prepareAction(job.triggerStatus === 1 ? 'stop' : 'start', job)"><Pause v-if="job.triggerStatus === 1" :size="14" /><Workflow v-else :size="14" />{{ t(job.triggerStatus === 1 ? '停止调度' : '启动任务') }}</button>
          ...
          <table v-if="view === 'users'"><thead><tr><th>ID</th><th>{{ t('用户名') }}</th><th>{{ t('角色') }}</th><th>{{ t('执行器权限') }}</th><th class="actions">{{ t('操作') }}</th></tr></thead><tbody><tr v-for="user in users" :key="user.id"><td class="mono">#{{ user.id }}</td><td><strong>{{ user.username }}</strong></td><td><span class="badge neutral">{{ t(user.role === 1 ? '管理员' : '普通用户') }}</span></td><td>{{ user.role === 1 ? t('全部') : user.permission || t('无') }}</td><td class="actions"><div class="row-actions"><button class="row-action" :disabled="!canWrite || user.username === info.username" @click="editUser(user)"><Pencil :size="14" />{{ t('编辑') }}</button><button class="row-action danger-text" :disabled="!canWrite || user.username === info.username" @click="removeUser(user)"><Trash2 :size="14" />{{ t('删除') }}</button></div></td></tr></tbody></table>
          <table v-if="view === 'logs'"><thead><tr><th>{{ t('日志 ID') }}</th><th>{{ t('任务 ID') }}</th><th>{{ t('触发时间') }}</th><th>{{ t('执行器地址') }}</th><th>Handler</th><th>{{ t('调度结果') }}</th><th>{{ t('执行结果') }}</th><th class="actions">{{ t('操作') }}</th></tr></thead><tbody><tr v-for="row in logs" :key="row.id"><td class="mono">#{{ row.id }}</td><td class="mono">#{{ row.jobId }}</td><td>{{ timeLabel(row.triggerTime) }}</td><td class="address-cell"><code>{{ row.executorAddress || '-' }}</code></td><td>{{ row.executorHandler }}</td><td><span class="badge" :class="row.triggerCode === 200 ? 'success' : row.triggerCode ? 'danger' : 'neutral'">{{ t(row.triggerCode === 200 ? '成功' : row.triggerCode ? '失败' : '待调度') }}</span></td><td><span class="badge" :class="row.handleCode === 200 ? 'success' : row.handleCode ? 'danger' : 'warning'">{{ t(row.handleCode === 200 ? '成功' : row.handleCode ? '失败' : '未完成') }}</span></td><td class="actions"><div class="row-actions"><button class="row-action" @click="detail = row; focusDialog()"><Search :size="14" />{{ t('查看详情') }}</button><button class="row-action" :disabled="!row.executorAddress || row.triggerCode !== 200" @click="openLog(row)"><FileText :size="14" />{{ t('读取执行日志') }}</button></div></td></tr></tbody></table>
